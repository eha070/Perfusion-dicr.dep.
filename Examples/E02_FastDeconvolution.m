...
%setup threshold
thres = .01; %probably needs to be tuned
...
[CBFest,phi,Iest,Cest,para] = perfusion1c.fastPerfusionAnalysis(Clow,AIFlow,timelineLow,thres,'mode','sSVD');

%reshape
CBFest = reshape(CBFest,m);
Iest   = reshape(Iest,[m,klow]);
Cest   = reshape(Cest,[m,klow]);
...
if showSingleCurve
    ...
    subplot(1,2,2);
    plot(timelineLow,s(Iest(pos(1),pos(2),:)),'LineWidth',3);
