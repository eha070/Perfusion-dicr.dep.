...
OIList = 0.01*[1, 1, 1, 1, 1];
voxelSizeFactorList = [1, 8, 16, 32, 64];
...
for i = (1:nVoxelSizes)
    ...
    voxelSizeFactor = voxelSizeFactorList(i);
    OI = OIList(i);
    ...
    RECirc = (CBFCirc-P)./P;
    REMS = (CBFMS-P)./P;
    meanREs.DeconvPerf(i) = mean(RECirc(:));
    meanREs.MSPerf(i) = mean(REMS(:));
    ...
    meanREs.DeconvPerfLoc(i) = mean(RECirc(:));
    meanREs.MSPerfLoc(i) = mean(REMS(:));
