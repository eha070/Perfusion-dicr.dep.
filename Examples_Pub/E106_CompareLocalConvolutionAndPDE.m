% ------------------------------------------------------------------------- 
%                      E106_CompareLocalConvolutionAndPDE
% 
% For perfusion parameters taken from a specific voxel of the PDE-Model,
% a 1D uptake curve is simulated using the local arterial input function
% with voxel-wise perfusion.
% 
...
if saveResultsForPublication
    ...
    CDataT = CData(idxT);
    CAnaT(CAnaT<1e-12)=0;
    CDataT(CDataT<1e-12)=0;

    %TIKZ
    figure(1);clf;
    plot(timeline(idxT),CAnaT,timeline(idxT),CDataT,'--');
    legend('Convolution','PDE');
    xlabel('time [s]');
    ylabel('concentration [mmol/mm^3]');
    matlab2tikz('ConvVsPDE.tikz', 'width', '\fwd'); 
end