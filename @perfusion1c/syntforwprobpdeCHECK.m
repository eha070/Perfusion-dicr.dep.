function [Cmat] = syntforwprobpdeCHECK(phimat,flowmat,Fmat,aifval,prm)
...
sstermini = Fmat; %ssterminini = SourceSingTermInitialization
% sstermini = sstermini/voxelvol;
time = 0;
for i = 2 : ntime    
    time = time + dt(i);
   
    cmath = cmat(:,:,:,i-1); %cmath = cmat-current
    ssterm = sstermini;
...
%     end;
    ...
    %for some reason flowmat>0 means p_{i+1} < p_{i}
    % flow from top
