...
\begin{document}
	...
	\subsection{Selected Results}
		...
		\end{alignat*}
		\CHalert{Assuming that $I(x,0)=0$ (from the looks of the impuls-response functions obtained from the data this should hold)}, we obtain:
		\[
...
% 		$$
% 		where $R(t) = \frac{1}{\phi}\int_0^t Q_{so}(x_0)du = \frac{tQ_{so}}{\phi}$ since $Q_{so}$ is independent of time in our case \CHalert{This would be valid only if $Q_{so}(x(s))$ is constant in $\Omega$. I was under the impression that $Q_{so}(x(s)) = 1$ for $s=0$ and $Q_{so}(x(s))=0$ for $s>0$. In this case $R(t)$ would become independent of $t$ and hence $R(t) = \frac{Q_{so}}{\phi}$}. This again can be rephrased as
% 		$$
% 		c(x_0,t) = \frac{Q_{so}(x_0)}{\phi}\int_0^{t}e^{-\frac{t-s}{T}}c_a(s)ds
% 		$$
% 		\CHalert{This would also only be valid if $Q_{so}(x)$ would be constant in $\Omega$.}
...
% 		where $P = |\Gamma|Q_{so}/|\Omega|$, becoming the total flow divided by the volume (ml/s/ml) which is perfusion. Thus, the classical model can estimate the \textit{total} perfusion $P$.
	...
	\begin{flushright}
		\signatureAl{7cm}{Erlend Hodneland, Erik Hanson\\University of Bergen}
		\signature{28ex}
	\end{flushright}
