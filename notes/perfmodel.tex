...
\section{Relating the simulated flow (SF) model with the classical model for perfusion}
We consider the solute transport equation (\ref{eq:conteqlocal}):
\begin{equation}
...
\begin{subequations}
	\label{eq:ourODE}
	\begin{alignat}{2}
		c'(s) + \frac{\Qso(s)}{\phi(s)}C(s) &= \frac{c_a(s)\Qso(s)}{\phi(s)}, & \quad c(0) &= 0 \label{eq:ourODEC} \\
		x'(s) &= \frac{q(s)}{\phi(s)}, &x(0) &= x_0 \label{eq:ourODEx}\\
		t'(s) &= 1,     &t(0) &= t_0, \label{eq:ourODEt}
	\end{alignat}
\end{subequations}
Here $x_0$ denotes the starting-point (in space) of the streamline and $t_0$ denotes the starting-timepoint of the streamline. $c(s)$ hence can be regarded the concentration curve of a particle which originated at $(x_0,t_0)$ and $(x(s),t(s))$ is the wavefront at artificial timepoint $s$. \CHalert{For a visualization see Fig. \ref{fig:Streamlines}}
\begin{figure}[h!tb]
...
Let the source term be non-zero within the ball $B_\epsilon$, zero elsewhere, and let $\partial B_\epsilon$ be the surface of $B_\epsilon$, corresponding to $x_0 = \{x : x_0 \in \partial B_\epsilon  \}$. All streamlines must connect with $\partial B_\epsilon$, and hence if we know the solution on $\partial B_\epsilon$ we know the solution everywhere on the streamlines. The obtained ODEs have the general solutions
% \begin{equation}
% c(x_0,t) = e^{-R(t)}\Big (\int_{t_0}^t e^{R(s)} F(x_0,s)ds + A_0\Big )
% \end{equation}
% for $R(s) := t G(x_0)$. Applying initial conditions $c(x_0,t_0) = 0$ for $t_0 = 0$ results in $A_0 = 0$.
% for $R(x_0,t) := t G(x_0)$. Applying initial conditions $c(x_0,t_0) = 0$ for $t_0 = 0$ results in $A_0 = 0$.
\[
	c(s) = \int_{0}^{s} e^{-(R(s)-R(v))}C_a(v)\frac{\Qso(v)}{\phi(v)} dv			
\]
for $R(v)=\int_{0}^v\Qso(x(u))/\phi(x(u)) d u$.
\CHalert{We can interpret $R(v)$ as the amount of contrast agent which is ,,accumulated'' while the particle moves through the source field.
Since in our setting $\Qso(x)=\qi$ for $x \in B_\epsilon$ and $\Qso(x)=0$ for $x \notin B_\epsilon$,  we find that $R(v)$ is constant if $v$ is bigger then some (streamline-dependent) threshold and hence.}
\\
BEGIN COMMENT HERE, ONLY FOR OUR UNDERSTANDING
\\
\begin{equation}
