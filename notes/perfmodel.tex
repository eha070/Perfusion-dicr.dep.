...
\begin{equation}
	...
	\label{eq:conteqlocal3}
\end{equation}
Following the method of characteristics and converting from Eulerican coordinates $(x(t),t)$ to Lagrangian coordinates $(x_0,t)$, where $x_0$ is fixed and $t$ is the parameterizatiton parameter, leads to the system of ODEs [Cite Evans]
\begin{subequations}
\begin{alignat}{2}
	\dot{c}(x_0,t) + G(x_0)c(x_0,t) &= F(x_0,t), & \qquad c(x_0,t_0) &= c_0(x_0) \label{eq:ODEC}\\
	\dot{x}(x_o) &= \frac{q(x_0)}{\phi(x_0)},&  x(x_0,t_0) &= x_0 \label{eq:ODEx}\\
\end{alignat}
\end{subequations}
for $G(x_0) := Q_{so}(x_0)/\phi(x_0)$ and the inhomogeneous term $F(x_0,t) := c_{so}(x_0,t)Q_{so}(x_0)/\phi(x_0)$ .The differentiation $\dot{c}$ is with respect to the time parameterisation $t$. Let the source term be non-zero within the ball $B_\epsilon$, zero elsewhere, and let $\partial B_\epsilon$ be the surface of $B_\epsilon$, corresponding to $x_0 = \{x : x_0 \in \partial B_\epsilon  \}$. All streamlines must connect with $\partial B_\epsilon$, and hence if we know the solution on $\partial B_\epsilon$ we know the solution everywhere on the streamlines. The obtained ODE has the general solution
\begin{equation}
