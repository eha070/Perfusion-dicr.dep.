...
\subsection{Relating the transport equation model with the traditional deconvolution model for perfusion}\label{sec:NewAndOld}
	...
	As a direct consequence, it follows by recursion that for a given voxel at location $i$ the concentration can be written as a convolution of the (global) arterial input function with the impulse response functions of all upstream voxels $\{1,\hdots,i-1\}$, i.e.
	\begin{equation}
		% C_i(t) = \phi_i W(J_1,\dots,J_{i-1})*\ca)(t).
		C_i(t) = \phi_i J_i * c_{\mathrm{in}})(t).		
		\label{eq:C_recur}
	\end{equation}
	where $W$ is some function which weights the impuls-response function according to the geometry of the flow and $J_i(s):=p_i\mathrm{exp}(- p_i(s))$ for $p_i=\Perfv^i/\phi_i$.
