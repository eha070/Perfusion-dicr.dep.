...
%	\tableofcontents
	...
	\begin{abstract}
		In perfusion imaging the usage of traditional one-compartment models to estimate hemodynamic parameters like blood flow (perfusion), blood volume and mean transit times is widespread. In this paper we show limitations of traditional models to recover perfusion. We derive theoretical details for voxel wise perfusion estimation when the image resolution reaches the spatial scale of the capillary systems.  %in coupled systems. 
...
%	%--------------------------------------------------
	...
	This definition most accurately reflects the physical perfusion at a given location and shows plausible perfusion values, cf. Figure~\ref{fig:flowpressureperfusion}. As an internal control, the average of $\Perfv$ was found to the \SI{50}{\siPml}, for all practical means identical to the global input perfusion. CONSTANTIN, CAN YOU PUT IN THE NUMBER?
	...
	\begin{figure*}[h!tb]\label{fig:RealData}
		\fwd = .29\textwidth
		\centering
		\begin{tabular}{p{\fwd} p{\fwd} p{\fwd} }
		 \includegraphics[width = \fwd]{./figs/real_AIF.pdf} & \includegraphics[width = \fwd]{./figs/real_axial160.pdf} & \includegraphics[width = \fwd]{./figs/real_meanC.pdf} \\
		...
		\end{tabular}
		\caption{Results from real-data experiments (see Sec. \ref{sec:RealData} for details on the data). (a) AIF manually selected from the MCA. (b) One slice of the voxel-wise CBF-reconstruction for a 3D volume of interest. (c) Mean concentration curve for the complete 3D volume of interest and the curve approximation by bSVD. CONSTANTIN: I THINK WE NEED LARGER AXIS LEGENDS ON THESE FIGURES. ALSO, WE NEED UNITS ON THE THE Y-AXIS ON THE RIGHT FIGURE. IT ONLY SAYS "CONCENTRATION" RIGHT NOW. I ALSO SUGGEST WE TAKE AWAY THE SMALL FIGURE LEGENDS. ITS ALL WRITTEN IN THE TEXT OF THE MAIN LEGEND.}
	\end{figure*}
	...
	In fact, in the most extreme example we measured overestimated perfusion values of up to $753\%$ (IS THIS RIGHT CONSTANTIN?).
