...
\newcommand{\siq}{\litre\per\second\per\square\meter}
\newcommand{\siqt}{\litre\per\second}
\newcommand{\siP}{\litre\per\second\per\litre}
\newcommand{\siQ}{\kilo\gram\per\second\per\litre}
\newcommand{\sirho}{\kilo\gram\per\litre}
\newcommand{\siJ}{\mol\per\second\per\square\meter}
\newcommand{\sic}{\mol\per\litre}
...
\begin{document}
	...
	\begin{table}[H]
	  ...
	  \caption{Parameters used in the numerical experiments, optimized for a slab of the capillary system in the human brain. In order to keep all parameters as simple as possible, permeability was assumed to be isotropic and permeability, porosity and viscosity were assumed constant.}		
		\begin{tabular}{ l  c  c  r }
			...
			Permeability  									& $k$ 				& $5 \cdot 10^{-6}$ 	& \si{\square\meter} 			\\	
			Porosity\cite{wu03}								& $\phi$ 			& $0.05$ 				& \si{\litre\per\litre}	\\	
			Spatial resolution 								& - 				& $(64,64,1)$ 			& -					\\
			Physical dimension 								& - 				& $(10,10,1)$ 			& \si{\meter}				\\
			Voxel size 										& - 				& $(0.156,0.156,1)$ 	& \si{\meter}	\\
	  \end{tabular}
	  \label{tab:par}
	\end{table}	
	...
	The convolution dataset was set up assuming voxel-wise well-mixed compartments described in Section \ref{sec:comp} and the perfusion values described in \eqref{eq:flux2perf}.
	...
	\begin{table}[H]
		\caption{Results of the perfusion restoration with median relative errors in percent. Relative error was computed block-wise as $\vert P_{\mathrm{rec}} - P_{\mathrm{true}}\vert / P_{\mathrm{true}}\cdot 100\%$. Displayed is the median RE over the entire domain. Abbreviations are: PMM - Porous Media Mode, ConvM - Convolution Model.}
		\centering
