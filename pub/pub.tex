...
More specifically, the common understanding of perfusion
	...
	However, it is not obvious that \eqref{eq:CBV} is valid also for a one-compartment field model where the voxels are feeding each other. 
	...
	\item In (A1) $J_i:[0,\infty)\to\R$ fulfills $\int_0^\infty J_i(s)\diffint s = 1$ and the local arterial input $c_{\mathrm{in},i}(t)$ is a weighted average of adjacent, upstream voxels $c_j(t)$, $J \subseteq \N$ and $\ca(t)$, if the voxel lies in the source field:
	\begin{equation}
		c_{\mathrm{in},i}(t) = \tfrac{1}{\Perfv}\Big(p_0 \ca(t) + {\textstyle\sum\limits_{j \in J}} p_jc_j(t)\Big),
	\end{equation}
	where $\Perfv=p_0 + \sum_{j\in J} p_j$ and $p_0>0$ if $i$ lies within the source field.
	\end{enumerate}
	...
	\begin{proof}
		...
		Let $I_k \subset \N$ denote the set of voxels which have $k$ layers of upstream voxels.
		...
		\begin{align*}	
			...
			&\stackrel{\mathclap{\text{(A2)}}}{=}  \tfrac{1}{\Perfv}\int_0^\infty \Big( p_0 \ca(s) + {\textstyle\sum\limits_{j \in J}}p_j c_j(s) \Big) \diffint s,\\
			&= \tfrac{1}{\Perfv} \Big(p_0 \int_0^\infty \ca(s) \diffint s + {\textstyle\sum\limits_{j\in J}} p_j \int_0^\infty c_{j}(s) \diffint s \Big) ,\\
			&\stackrel{\mathclap{\text{I.A.}}}{=}\tfrac{1}{\Perfv} \Big( p_0 + {\textstyle\sum\limits_{j\in 1}} p_j\Big)  \int_0^\infty\ca(s)\diffint s, \\
