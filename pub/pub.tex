...
%	\tableofcontents
	...
	Among the physiological parameters obtainable from tracer kinetic models, CBF has found particularly difficultly to describe reliably on a voxel-basis \cite{kudo10}.
	...
	We show that the discretized resulting 3D system can be equivalently described by a coupled set of multiple traditional one-compartment models, in line with the physical environment where each control volume is feeding the direct neighbors.
...
% In light of expected improvements in spatial resolution of medical scanners, traditional models will increasingly fail to accurately measure blood-flow when applied to patches of the capillary system. Instead, this establishes the need for continuous models as e.g. proposed in \cite{sourbron14}.
	...
	In this section we describe  the convolution- and the maximum slope model, two widely used one-compartment pharmacokinetic models for measurements of CBF and CBV \cite{kudo10,Feng2013,Chen2011}. 
	...
	In clinical practice this can be implemented by measuring $\ca$ in a feeding artery \cite{ostergaard96}.
...
In traditional models normalization is performed with respect to the volume and leads to the units of perfusion [\si{\siQmm}], cf. \cite{sourbron13}.
	...
	To avoid a discretization dependent flow field, for the PDE model we instead use vector valued surface fluid flux $q = q(x)$ with units [\si{\siq}], in agreement with geoscience and porous media simulation theory.
	...
	\begin{equation}
		...
		\label{eq:flowmodel}
	\end{equation}
	For the outward unit normal $n=n(x)$, the Neuman boundary conditions are reflecting zero fluid flux $q(x)$ across the boundary $\partial \Omega$.
...
\subsection{Relating the transport equation model with the traditional deconvolution model for perfusion}\label{sec:NewAndOld}
	...
	Then it holds that $P_{\mathrm{in}}=P_{\mathrm{out}}$ and we can describe $c_i$ by the classic model \eqref{eq:classicgeneral}:
	...
	\begin{equation}\label{eq:voxelcurve}
		C_i(t) = \phi_i(I_i*c_{\mathrm{in}})(t)
			\text{ for } J_i(t)=P_{\mathrm{in}}/\phi_ie^{- P_{\mathrm{in}}/\phi_it}.
	\end{equation}
	Here arterial input $c_{\mathrm{in}}$ is determined by \eqref{eq:cin}, which recursively depends on all upstream voxels until the global arterial input is reached.
...
These results show that the PDE model and the convolution model are equal if the convolution model is applied with the correct, local arterial input. 
Furthermore, we have shown that the impulse response function obtained by convolution of the global arterial input function is identical to an analytical recursive convolution along all upstream voxels. 
This clearly demonstrates that the traditional models measure a non-local perfusion quantity along the streamlines, but without accounting for the streamline length when normalizing with respect to volume. 
%Finally, these experiments rule out the error prone deconvolution itself as a major confounding factor in our experiments, since the red and the blue curves in Fig. \ref{fig:VoxelComp} were almost identical. Thus, the numerical challenges encounted by deconvolution are entirely apart from the challenges encountered by the problem of coupled flow.
	...
	Let us consider the recursive expression \eqref{eq:C_recur} for the contrast agent concentration, namely
	\begin{equation}
	...
	\end{align}
	where the second step is due to Fubini's theorem and $\ca(t)=0$ for $t<0$ and the third step follows since $\int_0^\infty \ca(s-u) \diffint u$ is constant in $s$.
...
%	%--------------------------------------------------
	...
	The global perfusion $\Perfs$ models perfusion along the streamlines and most accurately reflects the physical notion of volume flow within the correct distribution volume.
	...
	The reason for this failure is not numerical instabilities in the deconvolution, but that perfusion becomes over estimated since the traditional models will not account for correct distribution volume.
	...
	We expect to find overestimation also in pathological tissue, where fluid is passing through multiple areas and recommend to be take this effect into consideration in clinical evaluation of the data.
