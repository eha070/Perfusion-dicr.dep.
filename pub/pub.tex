...
%	\tableofcontents
	...
	\begin{abstract}
		...
		Methods:
		In this work we derive theoretical details for voxelwise perfusion estimation when the image resolution reaches the spatial scale of the capillary systems. We analyse the transitional understanding of perfusion estimation using a continuous model for propagation of a tracer in the capillary tissue. The effects are illustrated using  porous media flow simulations.
		Results:
		We showed how traditional one-compartment models only are equivalent to a continuos model for tracer transport, when upstream effects can be counted for. This is however difficult to incorporate practical applications. 
...
%These results also support the idea presented in the next section that pointwise perfusion can be regarded a globally constant quantity along the streamlines, as it can only be estimated when the entire streamline characteristics are known.
	...
	\subsection{Relation Between Flow to Perfusion}\label{sec:flux2perf}
...
In this section we will describe a simulation experiment illustrating the effects outlined in Section \ref{}.
