...
\subsection{Relating the transport equation model with the traditional deconvolution model for perfusion}\label{sec:NewAndOld}
	...
	As a direct consequence, it follows by recursion that for a given voxel at location $i$ the concentration can be written as a convolution of the (global) arterial input function with the impulse response functions of all upstream voxels $\{1,\hdots,l-1\}$, i.e.
	\begin{equation}
		C_l(t) = \phi_l \cdot ((J_1*\dots *J_l)*\ca)(t).
		\label{eq:C_recur}
	\end{equation}
