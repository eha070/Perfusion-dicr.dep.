...
%	\tableofcontents
	...
	\begin{abstract}
		\textbf{Objective:}
		In perfusion imaging the usage of traditional one-compartment models to estimate haemodynamic parameters like perfusion, blood volume and mean transit times is widespread. In this paper we set up a mathematical framework revealing limitations of traditional models related to perfusion recovery.
		\textbf{Methods:}
		...
		\textbf{Conclusion:}
		Results indicate that traditional models for perfusion overestimate blood flow when applied to only parts of a coupled system. Estimation of the blood volume is stable and can also be applied in coupled systems.
		\textbf{Significance:}
	...
	\rtNote{Highlighted that model is valid for intravascular contrast agents.}
...
According to the common understanding of perfusion,
    ...
    \begin{figure*}[h!tb]
    	...
    	\fwd = .46\textwidth
    	\begin{tabular}{c c}
    		\includegraphics[width=\fwd]{figs/E110_CBFOnDifferentResolutions_plot-Ps.eps} & \includegraphics[width=\fwd]{figs/E110_CBFOnDifferentResolutions_plot-Pv.eps}\\
    		(a) Global perfusion $P_s$ & (b) Local perfusion $P_v$
    	\end{tabular}
    	\caption{Comparing reconstructed perfusion to ground truth perfusion as a function of varying voxel size. Dashed, blue lines are absolute perfusion values (left axis), solid, red lines are relative errors (right axis). (a) Global perfusion $P_s$ is independent of discretization. For a voxel size corresponding to the entire ROI (voxel size = \SI{3}{\milli\meter}) the reconstructed perfusion maps $P_{bSVD}$ and $P_{MS}$ are close to the ground truth perfusion $P_s$. Subdivision of the domain with smaller voxel sizes leads to a substantial over-estimation of perfusion for both reconstruction methods. (b) Local perfusion $P_v$ is depending on discretization. For a voxel size corresponding to the entire ROI (voxel size = \SI{3}{\milli\meter}) the reconstructed perfusion maps $P_{bSVD}$ and $P_{MS}$ are close to the ground truth perfusion $P_s$. A further subdividion of the domain leads to substantial under-estimation of perfusion for both reconstruction methods.}
