...
\begin{document}
	...
	To solve \eqref{eq:flowmodel} we used the two-point flux approximation finite volume method (TPFA). 
	Based on mass balance equations, we calculated the flow in such a fashion that conservation of mass was guaranteed.
	The transport equation \eqref{eq:conteqlocal} was solved in a similar fashion by a straight-forward discretization of \eqref{eq:conteq}.

	%--------------------------------------------------
	% Subsection: Estimate the Porosity
	%--------------------------------------------------	
	\subsection{Discretization of the single phase flow model} \label{sec:flowmodel}
