...
%	Details of the synthetic model are presented in Section \ref{sec:transport}.	
	...
	\begin{equation}
		q = -\frac{K}{\mu} \left( \nabla p + \rho g  \nabla z \right).
	\end{equation}
	Here $g$ is the gravitational acceleration, $K$ is the permeability tensor [\si{\square\meter}], $z$ is the spatial position along the gravitational field and $\mu = \mu(x)$ [$\si{\pascal\second}$] is the viscosity of the fluid.
	...
	A simplified version of Darcy's law thus becomes
	\begin{equation}
		q = -\frac{K}{\mu} \nabla p.
		\label{eq:syntdarcysimp}
	\end{equation}
	Equations \eqref{eq:syntcontsimp2} and \eqref{eq:syntdarcysimp} can be combined, and further we assume that $K$ is a symmetric and positive definite tensor such that $K_{ii} = k$, and zero elsewhere.
	This yields the following elliptic partial differential equation in the pressure-field $p$ [\si{\pascal}],
	\begin{equation}
		\left\vert
		\begin{alignedat}{2}
	...
	Note that \eqref{eq:flowmodel} exhibits a solution which is unique up to constant since only pressure differences are taken into consideration \cite{evans98}.
