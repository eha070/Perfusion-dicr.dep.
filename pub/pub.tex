...
In light of expected improvements in spatial resolution of medical scanners, traditional models will increasingly fail to accurately measure blood-flow when applied to patches of the capillary system. Instead, this establishes the need for continuous models as e.g. proposed in \cite{sourbron14}.
	...
	Let $\Omega_i$ be an arbitrary control volume with one inlet and one outlet, and let $C(t)$ denote the average CA concentration within $\Omega_i$ at timepoint $t$.
...
% 	with the impulse response function $I(t)=\Pa e^{-t/T}$.
	...
	The validity of the traditional methods rely on a ROI having only one inlet and one outlet, and that transition times are prescribed by some probability distribution.
	...
	Assuming stationary, incompressible flow at low Reynold numbers within the capillary system \cite{Cho2011}, the flow can be described by Darcy's law \cite{Darcy56}. For simplicity, we also assume an isotropic porous media.
	...
	A major difference between the described flow-model model and traditional tracer kinetic modeling is the normalization of the flow field within the traditional models,
