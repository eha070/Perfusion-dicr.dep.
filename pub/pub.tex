...
%\newcommand{\milli\meter\cubed\per\second\per\milli\meter\squaredtilde}{\milli\gram\per\second\per\milli\meter\cubed}
...
From the porous media model using \eqref{eq:flowmodel} and \eqref{eq:conteqlocal}, streamlines to compute global perfusion $P_s$ were found from tracking of the flux vector field $\mathbf{q}$ by FACT \cite{Mori2002}, known from  tractography.
...
%\end{figure*}	
	...
	We performed two comparisons of the restored flow to ground truth flow: Volume- and surface normalized flow. For volume normalization, the absolute flow $F$ [\SI{}{\milli\liter\per\minute}] was normalized to a distribution volume of 100 \SI{}{\milli\liter}. This comparison is equivalent to traditional perfusion. %Average flow over the entire domain $\Omega$ is denoted $\overline{F}$. 
