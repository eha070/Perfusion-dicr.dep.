...
%	Since the CA transport itself is driven mainly by blood flow, we will describe a simple model for the blood flow through capillary tissue in Section \ref{sec:flow}.
	Within the porous capillary system we expect the blood flow to be driven mainly by pressure differences. We therefor also set up a PDE flow modell using the  continuity equations and Darcy's law \missingsource. Traditional tracer kinetic modelling is using volume normalized fluid flow with units [\si{\siQmm}] as a quantity to describe fluid transport. 
...
\subsection{Relating the transport equation model with the classical deconvolution model for perfusion}
	...
	We follow the method of characteristics and convert from Eulerian coordinates $(x,t)$ to Lagrangian coordinates $(x_0,t_0,s)$.
	Here $(x_0,t_0)$ corresponds to the beginning of a streamline at point $x_0$ timepoint $t_0$. 
	...
	\begin{equation}\label{eq:Sol}
		c(x(s),t(s)) = \int_{0}^{s} e^{-(R(s)-R(v))}c_{so}(v)\frac{\Qso(v)}{\phi(v)} \diff v.
	\end{equation}
	where $R(v)=\int_{0}^v\Qso(x(u))/\phi(x(u)) d u$.

	We now impose the additional assumptions that $\Qso$ and $\phi$ are constant along the streamline up to timepoint $t$.
	Following the path of a particle emerging at timepoint $t_0=0$ and using that in this case $t=s$ from \eqref{eq:ODEt}, one can see that \eqref{eq:Sol} becomes
...
The observed over-estimation of perfusion is also predicted by the theoretical analysis in Sec. \ref{sec:flux2perf}, stating that the perfusion will be over-estimated with a factor proportional to the length of the streamlines. 
