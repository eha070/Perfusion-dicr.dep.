...
\title{Perfusion in Coupled Systems}
...
%	\tableofcontents
	...
	\begin{abstract}
		In perfusion imaging the usage of traditional one-compartment models to estimate hemodynamic parameters like blood flow (perfusion), blood volume and mean transit times is widespread. In this paper we show limits of traditional models to recover perfusion. We derive the theoretical details related to perfusion estimation in coupled systems, as e.g. the capillary system.
...
As a remedy, we propose to define perfusion in coupled systems as laminar flow along the streamlines.
	\end{abstract}
	...
	In \cite{Henkelman1990} the problem of scale dependent continuous perfusion was discussed. Perfusion defined as flow entering a voxel will scale with voxel size as volume flow (perfusion) scales differently than surface flow (flux). 
	This notion of perfusion is therefore unsuited.	
	More recently, a two-compartment model for voxelwise perfusion was introduced in \cite{sourbron14} as a mathematical definition of perfusion.	
	...
	We do this by simulating a tissue patch with one inlet, one outlet, and a highly coupled system as e.g. pure capillary system in between.
	...
	The traditional model assumes that the change of concentration at timepoint $t$ can be described by the ordinary differential equation 
...
In traditional models normalization is performed with respect to the volume and leads to the units of perfusion [\si{\siQmm}], cf. \cite{sourbron13}.
	...
	Let $V_i$ be the volume of $\Omega_i$ and $v_i$ the the blood volume within $\Omega_i$.
...
\subsection{Relating the transport equation model with the traditional deconvolution model for perfusion}\label{sec:NewAndOld}
	...
	\end{equation}
	This allows us to define the perfusion within $\Omega_i$ as the total fluid inflow divided by the voxel volume,
	...
	\begin{equation}
		...
		\label{eq:C_recur}
	\end{equation}
	For a discretization using an upwind scheme we have the special case $J_j(s):=p_i\mathrm{exp}(- p_i(s))$ and $p_i=\Perfv^i/\phi_i$.
...
This clearly demonstrates that the traditional models measure a non-local perfusion quantity along the streamlines, and the entire streamline length within the capillary system is not taken into consideration where the blood is providing the tissue with nutrients and oxygen. These results also support the idea presented in the next section that pointwise perfusion can be regarded a global quantity along the streamlines, as it can only be estimated when the entire streamline characteristics are known.
	...
	This problematic issue was already identified by \cite{Henkelman1990}, stating that normalizing the perfusion by control volumes that are not upstream of the flow will lead to multiple counting of the same flow, and hence an over-estimation of perfusion.
	...
	\end{equation}
	Let $R:S\to\R^+$ denote $r:[0,l]\to\R^+$ in cartesian coordinates, i.e. $R(y):=r(u)$ for $y=s(u)$. We define the perfusion at an arbitrary point $y$ on the streamline as
	\begin{equation}
	...
	\begin{equation}
		F(y,\varepsilon R)
		= \varepsilon^2 \Big(\pi + \varepsilon \int_{B_{1}(0)} \nu^\top J(\zeta)zR^3 \diffint z\Big)
	\end{equation}
	where $\zeta \in (0,z)$ and simplifications are due to $R = 1/\sqrt{\vert q(y)} \vert$ and $\nu:=q(y)/\vert q(y) \vert$.
	It follows from \eqref{eq:perfusiondef} that
	\begin{equation}\label{eq:flux2perf}
		\Perfs= \left(\int_0^l r(s)^2 \diffint s\right)^{-1}
	\end{equation}
...
%	%--------------------------------------------------
	...
	In this work we have studied the impact of traditional 1C models to perfusion reconstruction in a coupled system.
